...
var CONFIG = {
  SPREADSHEET_ID: '1uRPfCjExSQla22f7hASHW7zbmawRx1UgdsodB3XJyaY',
...
// ── RESPONSE HELPER ────────────────────────────────────────────
// Apps Script doesn't support custom CORS headers, but deployed web apps
// with "Anyone" access automatically allow cross-origin GET requests.
// POST requests need mode:'no-cors' on the client side.
...
// JSONP support for GET requests (bypasses CORS completely)
...
function doGet(e) {
  ...
  try {
    ...
    // Read actions
    if (action === 'getVocab') {
      result = getVocab(e.parameter);
    } else if (action === 'getDeck') {
      result = getDeck(e.parameter.studentId);
    // Write actions via GET+payload (JSONP workaround)
    } else if (action === 'savePreTest') {
      var data = JSON.parse(e.parameter.payload || '{}');
      result = savePreTest(data);
    ...
    return jsonpResponse({ error: err.toString() }, callback);
  }
}
