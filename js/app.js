...
//  FS-101 Leitner Box | Profe. Cipriani | Monmouth University
// ============================================================
...
function showMyCards() {
  ...
  deck.forEach(card => {
    const b = Math.min(parseInt(card.box) || 1, 5);
    boxes[b].push(card);
  });

  // Sort each box alphabetically by Spanish word
  Object.values(boxes).forEach(arr => {
    arr.sort((a, b) => a.spanish.localeCompare(b.spanish));
  });
  ...
  const boxColors = {
    1: '#FDF0F0', 2: '#FEF9E7', 3: '#EBF3FB', 4: '#EBF7ED', 5: '#F5EEF8'
  };

  const boxTextColors = {
    1: '#7B2C2C', 2: '#7D6608', 3: '#1F4E79', 4: '#1E6B2E', 5: '#4A235A'
  };

  // Build box filter tabs
  let tabHtml = '';
  for (let i = 1; i <= 5; i++) {
    ...
    tabHtml += `<button
      ...
      id="boxTab${i}"
      style="width:auto;padding:5px 12px;font-size:12px;background:${boxColors[i]};
             color:${boxTextColors[i]};border-color:${boxTextColors[i]};margin-bottom:0"
      onclick="filterMyCards(${i})">
      Box ${i} <span style="font-weight:400">(${count})</span>
    </button>`;
  }
  document.getElementById('boxTabs').innerHTML = tabHtml;

  // Store boxes globally for filtering
  window._myCardsBoxes     = boxes;
  window._myCardsNames     = boxNames;
  window._myCardsColors    = boxColors;
  window._myCardsTextColors = boxTextColors;

  // Show all boxes by default
  renderMyCards(0);
  document.getElementById('myCardsTitle').textContent =
    S.name + ' — ' + deck.length + ' cards total';
...
function filterMyCards(boxNum) {
  ...
  for (let i = 1; i <= 5; i++) {
    const tab = document.getElementById('boxTab' + i);
    if (tab) tab.style.fontWeight = i === boxNum ? '800' : '400';
  }
  renderMyCards(boxNum);
}

function renderMyCards(filterBox) {
  const boxes      = window._myCardsBoxes;
  const boxNames   = window._myCardsNames;
  const boxColors  = window._myCardsColors;
  const textColors = window._myCardsTextColors;
  ...
  for (let b = 1; b <= 5; b++) {
    ...
    const cards = boxes[b];
    if (cards.length === 0 && filterBox === 0) continue; // hide empty boxes in "all" view

    html += `<div style="margin-bottom:16px">
      <div style="
        background:${boxColors[b]};
        color:${textColors[b]};
        font-weight:700;
        font-size:12px;
        padding:8px 12px;
        ...
        align-items:center;
      ">
        <span>BOX ${b} — ${boxNames[b].toUpperCase()}</span>
        <span style="font-weight:400">${cards.length} card${cards.length !== 1 ? 's' : ''}</span>
      </div>`;

    if (cards.length === 0) {
      html += `<div style="padding:10px 12px;font-size:13px;color:#888;
               background:#fafafa;border-radius:0 0 8px 8px;font-style:italic">
        No cards in this box yet
      </div>`;
    } else {
      html += `<div style="border:1.5px solid ${boxColors[b]};border-top:none;
               border-radius:0 0 8px 8px;overflow:hidden">`;

      // Group by word (pair ES→EN and EN→ES together)
      const wordMap = {};
      cards.forEach(card => {
        if (!wordMap[card.wordId]) wordMap[card.wordId] = { spanish: card.spanish, english: card.english, cards: [] };
        wordMap[card.wordId].cards.push(card.direction);
      });

      Object.values(wordMap).forEach((word, idx) => {
        const isEven = idx % 2 === 0;
        const bg     = isEven ? '#ffffff' : '#fafafa';
        const dirs   = word.cards.join(' · ');
        const hasPersonal = S.myTranslations[Object.keys(wordMap).find(k => wordMap[k] === word)] || [];
        ...
        ">
          <div>
            <span style="font-weight:600;color:#1a1a2e">${word.spanish}</span>
            <span style="color:#888;margin:0 6px">→</span>
            <span style="color:#555">${word.english || '—'}</span>
            ${hasPersonal.length > 0
              ? `<span style="font-size:10px;color:#7D3C98;margin-left:6px">+${hasPersonal.length} personal</span>`
              : ''}
          </div>
          <span style="font-size:10px;color:#aaa;white-space:nowrap;margin-left:8px">${dirs}</span>
        </div>`;
      });
