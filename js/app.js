...
function renderMyCards(filterBox) {
  ...
  for (let b = 1; b <= 5; b++) {
    ...
    if (cards.length === 0) {
      ...
      const wordMap = {};
      cards.forEach(card => {
        if (!wordMap[card.wordId]) wordMap[card.wordId] = { wordId: card.wordId, spanish: card.spanish, english: card.english, cards: [] };
      ...
      Object.values(wordMap).forEach((word, idx) => {
        ...
        const hasPersonal = S.myTranslations[word.wordId] || [];
